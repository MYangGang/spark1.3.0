...
/**
 * 实现了面向stage调度机制的高层次调度层。它会为每个job计算一个stage的DAG（有向无环图），追踪RDD和stage的
 * 输出是否被物化（写入了磁盘和内存等地方），并且寻找一个最优的调度机制来运行job。它会将stage做TaskSets提交
 * 到底层的TaskSchedulerImpl，来在集群上运行它们（task）。
 *
 * 除了处理stage的DAG，它还负责决定运行每个task的最佳位置，基于当前的缓存状态，将这些最佳的位置发送给底层的
 * TaskSchedulerImpl。此外，它还会处理由于shuffle输出文件掉失导致的失败，在这种情况下，旧的stage可能就会被
 * 重新提交。一个stage内部的失败，如果不是shuffle文件掉失所导致的，会被TaskScheduler处理，它会多次重试每一
 * 个task，直到最后，实在不行了，才会取取消整个stage。
 */
...
class DAGScheduler(
      ...
      properties: Properties = null)
  {
    ...
    if (finalStage != null) {
      ...
      //也就是说这个job的最后一个stage，也就是我们的finalStage了
      ...
      if (shouldRunLocally) {
        ...
        //并且导致其它所有的stage，都给放入watingStages队列中
  ...
  /**
    * 提交stage的方法
    * stage划分算法的入口
    * stage的划分算法，是由submitStage（）方法和getMissingParentStages（）方法共同组成的
    */
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        //那么，此时，就会去首先提交第一个stage，stage0
        //其余的stage，全部都在waitingStages里面
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    if (tasks.size > 0) {
      ...
      //最后，针对stage的task，创建TaskSet对象，调用taskScheduler的submitTasks()的方法，提交TaskSet
  ...
  /**
    ...
    * 其实就是从stage的最后一个rdd开始，去找，哪个rdd的partition，是被cache了，或者checkpoint了
    ...
    : Seq[TaskLocation] =
  {
    ...
    //最后，递归调用自己，去寻找rdd的父rdd，看看对应的partition是否缓存或checkpoint
    ...
    //如果这个stage，从最后一个rdd，到最开始的rdd，partition都没有被cache或checkpoint
