...
/**
 * 1、底层通过操作一个SchedulerBackend，针对不同的集群类型（Standalone、yarn、mesos），调度task
 * 2、它也可以通过使用一个LocalBackend，并且将isLocal参数设置为true，来在本地模式下工作
 * 3、它负责处理一些通用的逻辑，比如说决定多个job的调度顺序，启动推测任务的执行
  ...
  extends TaskScheduler with Logging
{
  ...
  override def submitTasks(taskSet: TaskSet) {
    ...
    //SparkContext的原理剖析时，创建TaskScheduler时，为TaskSchedulerImpl创建SparkDeploySchedulerBackend
    //这里的backend，指的就是之前创建好的SparkDeploySchedulerBackend
    //而且这个backend是负责创建AppClient，想Master注册Application的
    ...
    for (i <- 0 until shuffledOffers.size) {
      ...
      if (availableCpus(i) >= CPUS_PER_TASK) {
        try {
          ...
          //调用TaskSetManager的reosourceOffer()方法，
          //去找到，在这个executor上，就用这种本地化级别，taskset哪些task可以启动
          //遍历，使用当前本地化级别，可以在该executor上启动的task
          for (task <- taskSet.resourceOffer(execId, host, maxLocality)) {
            //放入tasks这个二维数组，给指定的executor加上要启动的task
            tasks(i) += task
            //到这里为止，task的分配算法，其实就算实现了
            //我们尝试着用本地化级别这种模式，去优化task的分配和启动，优先希望在最佳本地化的地方启动task
            //然后，将task分配给executor
  ...
  def resourceOffers(offers: Seq[WorkerOffer]): Seq[Seq[TaskDescription]] = synchronized {
    ...
    //首先，将可用的executor进行shuffle，也就是说进行打散，从而做到，尽量可以进行负载均衡
    val shuffledOffers = Random.shuffle(offers)
    // Build a list of tasks to assign to each worker.

    //然后针对WorkerOffer，创建出一堆需要用的东西
    //比如tasks，很重要，可以理解为一个二维数组，ArrayBuffer，元素又是一个ArrayBuffer
    //并且每个子ArrayBuffer的元素数量是固定的，也就是这个executor可用的cpu数量
    val tasks = shuffledOffers.map(o => new ArrayBuffer[TaskDescription](o.cores))
    val availableCpus = shuffledOffers.map(o => o.cores).toArray

    //这个很重要，从rootPool中取出了排序的TaskSet
    //之前TaskScheduler初始化的时候，先创建完TaskSchedulerImpl和SparkDeploySchedulerBackend之后，
    //执行一个initialize()方法，在这个方法中，其实会创建一个调度池
    //这里，相当是说，所有提交的TaskSet，首先，会放入这个调度池
    //然后在执行task分配算法时候，会从这个调度池中，取出排好对的TaskSet
    ...
    //本地化级别，有几种：PROCESS_LOCAL，进程本地化，rdd的partition和task，进入一个executor内，那么速度当然快了
    //NODE_LOCAL，节点本地化，rdd的partition和task，在一个worker节点上
    //NO_PREF，没有所谓的本地化级别
