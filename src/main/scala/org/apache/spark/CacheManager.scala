...
private[spark] class CacheManager(blockManager: BlockManager) extends Logging {
  ...
  def getOrCompute[T](
    ...
    blockManager.get(key) match {
      ...
      //也不在远程的BlockManager的本地内存或磁盘
      //那么就进行后续的处理
      case None =>
        // Acquire a lock for loading this partition
        // If another thread already holds the lock, wait for it to finish return its results
        //再次调用一次BlockManager()的get()方法，去获取数据
        //如果获取到了，就直接返回
        //还是没有获取到，那么往后走
        ...
        try {
          ...
          //但是如果rdd没有checkpoint过，那么就使用父rdd的数据，执行算子，计算一份
          ...
          //只是因为某些原因，持久化过的数据没有找到，才走到这里来的
          //所以读取了checkpoint数据，或着重新计算数据之后，要用putInBlockManager()方法
  ...
  private def putInBlockManager[T](
    ...
    if (!putLevel.useMemory) {
       ...
       */
      updatedBlocks ++=
        //直接调用BlockManager的putIterator()方法，直接将数据写入磁盘
    ...
    //如果指定了内存级别
      ...
      //如果unrollSafely()方法判断是数据可以写入内存，就将数据写入内存
      //但是如果unrollSafely（）方法判断某些数据，无法写入内存，就只能写入磁盘了
      blockManager.memoryStore.unrollSafely(key, values, updatedBlocks) match {
        ...
        case Right(it) =>
          ...
          //如果有些数据，实在是无法写入内存，那么就判断，数据是否有磁盘级别
          //如果有的话，就将数据写入磁盘级别
          if (putLevel.useDisk) {
