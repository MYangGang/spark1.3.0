...
private[hash] object BlockStoreShuffleFetcher extends Logging {
    ...
    : Iterator[T] =
  {
    ...
    //然后调用getServerStatuses()方法，传入参数shuffleId和reduceId
    //shuffleId可以代表当前这个stage的上一个stage，我们知道shuffle是分为两个stage的
    //shuffle write发生在上一个stage中，shuffle read是发生在当前的stage中
    //这么来理解
    //首先通过shuffleId可以限制到上一个stage的所有ShuffleMapTask的输出的MapStatus
    //接着，通过reduceId，也就是bucketId，来限制，从每个MapStatus中，来获取当前这个ResultTask需要
    //获取的每个ShuffleMapTask的输出文件的信息
    //这个getServerStatuses()方法，一定是走远程网络通信的，因为要联系Driver上的DAGScheduler的
    // MapOutputTrackerMaster
    val statuses = SparkEnv.get.mapOutputTracker.getServerStatuses(shuffleId, reduceId)
    logDebug("Fetching map output location for shuffle %d, reduce %d took %d ms".format(
      shuffleId, reduceId, System.currentTimeMillis - startTime))

    //对刚才拉取到的信息，statuses，进行一些数据结构上的转换操作
    //弄成map格式的数据
