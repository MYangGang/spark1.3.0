  ...
  extends Logging
{
  ...
  def launchTask(
    ...
    //对每一个task，都会创建一个TaskRunner线程
    val tr = new TaskRunner(context, taskId = taskId, attemptNumber = attemptNumber, taskName,
      serializedTask)
    //将TaskRunner，放入内存缓存中
    runningTasks.put(taskId, tr)
    //threadPool，Executor内部的一个java线程池
    //这里其实是，将task封装在一个TaskRuner的线程中
    //直接将线程丢入线程池中，进行执行
    //这里我们要知道，线程池是自动实现了排队机制的，也就是说，如果线程池内的线程暂时没有空闲的
    //那么丢进来的线程都是要排队的
  ...
  class TaskRunner(
    ...
    override def run() {
      ...
      try {
        ...
        //包括运行了多长时间、反序列化耗费了多长时间、java虚拟机gc耗费了多长时间、结果的序列化耗费了多长时间
        //这些东西会在我们的SparkUI上显示，也就是我们的4040端口
        for (m <- task.metrics) {
        ...
        //调用了Executor所在的CoraseGrainedExecutorBackend的statusUpdate（）方法
  ...
  private def updateDependencies(newFiles: HashMap[String, Long], newJars: HashMap[String, Long]) {
    ...
    //用Java的synchronized进行了多线程并发访问的同步
    //因为task实际上是以java线程的方式，在一个CoarseGrainedExecutorBackend进程内并发运行的
    //如果在执行业务逻辑时，要访问一些共享的资源
    //那么就可能会出现多线程并发访问安全问题
    //所以在这里，Spark选择进行了多线程并发访问的同步（synchronized）
    //为什么在这里要进行多线程并发访问的同步？因为在这里访问了，诸如currentFiles等等，这类共享的资源
    synchronized {
